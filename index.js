...
function meleeRangedGrouping(str) {
  const splittedHeroes = str.length > 0 ? splitting(str) : [];
  const types = [...new Set(splittedHeroes.map((hero) => hero[1]))];
  const newArr = types.map((type) => {
    const newHeroes = splittedHeroes
      .filter((hero) => hero[1] === type)
      .map((hr) => hr[0]);
    return newHeroes;
  });
  return newArr;
...
function deepSum(arr) {
  ...
  for (let index = 0; index < arr.length; index++) {
    const innerArray = arr[index]; //array dimensi 2: 2 dimensi
    innerArray.forEach((elemen) => {
      elemen.forEach((inElem) => (sum += inElem));
    });
  }
  return sum;
}
